...
lastmod = 2021-04-19T10:10:29+08:00
tags = ["flutter"]
draft = false
from = "orgmode"
+++

从零快速搭建flutter APP脚手架，集成常用的必备功能以及示例，这样以后新建app的时候也快，可以快速地得到一个可以运行的版本；我目前使用flutter使用的是 `dev channel` ，因此这个脚手架会随着版本的更新而快速更新。
...
由于我这里是搭建的脚手架，为了后面引入其它文件的时候方便，这里我们取名为 **net.dieya.app** ，这个参考 [修改APP基本信息](#修改app基本信息)是可以通过命令来修改的。
...
生成的图放到 **android/app/src/main/res** 目录下面，
...
在sketch里面安装插件[sketch measure](https://github.com/qjebbs/sketch-meaxure)，然后按照下面的方式来进行导出。

<div align="center">
  <div></div>

<img src="/ox-hugo/_20210224_095010Untitled.png" alt="_20210224_095010Untitled.png" width="30%" align="center" /><img src="/ox-hugo/_20210224_095143Untitled.png" alt="_20210224_095143Untitled.png" width="30%" align="center" />
...
三倍图一般命名是file@3x，我们只需要把这些文件放在一个单独的目录下面，然后在这个目录下面通过下面的脚本来执行就可以生成对应的@2x以及一倍图文件:
...
**注意** :
无论如何一倍图的目录下面需要有对应的文件，要不然flutter会报找不到这个资源，仅仅有三倍图而一倍图没有的话flutter会找不到这个文件，除非显式地引用三倍图的目录
